...
function NavBar() {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const groups = userGroupsSnapshot.val();
        const userGroups = Object.entries(groups)
          .filter(([_, group]) => group.members?.[currentUser.uid])
          .map(([id, _]) => id);
        
        // Listen for join requests in user's groups
        const requestsRef = ref(database, 'groups');
  ...
  const isActive = (path) => {
    // Special handling for home and dashboard paths
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex">
            ...
            {/* Desktop Navigation */}
            <div className="hidden sm:ml-6 sm:flex space-x-4">              <Link 
                to="/dashboard" 
            ...
            {currentUser && (
              <div className="relative mr-2">
                <button
                  ...
                  className="p-2 rounded-full text-ios-gray dark:text-ios-dark-text-secondary hover:bg-ios-gray6 dark:hover:bg-ios-dark-secondary transition-colors"
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-80 rounded-ios shadow-lg bg-white dark:bg-ios-dark-elevated border border-ios-gray6 dark:border-ios-dark-border ring-1 ring-black ring-opacity-5 z-50">
                      ...
                      {notifications.length > 0 ? (
                        notifications.map(notification => (
                          <button
                            ...
                            className={`w-full text-left px-4 py-3 hover:bg-ios-gray6 dark:hover:bg-ios-dark-secondary transition-colors ${
                              !notification.read ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                            }`}
                          >
                            <div className="flex items-start">
                              <div className="flex-shrink-0">
                                <div className="w-8 h-8 bg-ios-blue dark:bg-ios-teal rounded-full flex items-center justify-center text-white">
                                  <FaUsers className="h-4 w-4" />
                                </div>
                              </div>
                              <div className="ml-3 flex-1">
                                <p className="text-sm font-medium text-ios-black dark:text-ios-dark-text">
                                  {notification.userName} requested to join {notification.groupName}
                                </p>
                        ...
                        <div className="px-4 py-3 text-center text-ios-gray dark:text-ios-dark-text-secondary">
                          No notifications
                        </div>
                      )}
                    </div>
