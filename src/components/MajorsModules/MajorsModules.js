...
import { FaFilter, FaSearch, FaStar, FaChevronDown, FaChevronUp, FaGraduationCap, FaChevronRight } from 'react-icons/fa';
...
const MajorsModules = () => {
  ...
  const [departmentDisplayMode, setDepartmentDisplayMode] = useState('collapsed');
  
  // Enhanced categorization state
  ...
  const [filterOptions, setFilterOptions] = useState({
    level: 'all', // 'all', 'undergraduate', 'postgraduate'
  ...
  useEffect(() => {
    ...
    if (activeTab === 'majors' || activeTab === 'all') {
      ...
      setFilteredMajors(matchedMajors);
    }
      ...
      <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white dark:bg-ios-dark-elevated ios-card shadow-sm rounded-xl overflow-hidden">
            ...
            {filteredMajors.length > 0 ? (
              ...
              <div className="p-8 text-center">
                <p className="text-ios-gray dark:text-ios-dark-text-secondary mb-2">No programs found</p>
        ...
        {activeTab === 'modules' && view === 'list' && !searchTerm && (
          <div className="space-y-4">
            ...
            {categorizedModules.favorites?.modules.length > 0 && (
              <div className="bg-white dark:bg-ios-dark-elevated ios-card shadow-sm rounded-xl overflow-hidden">
                <div className="px-4 py-3 flex items-center border-b border-ios-gray6 dark:border-ios-dark-border">
                  <FaStar className="text-yellow-400 mr-2" />
                  <h2 className="font-medium dark:text-ios-dark-text">Your Favorite Modules</h2>
                </div>
                
                <ul className="divide-y divide-ios-gray6 dark:divide-ios-dark-border">
                  {categorizedModules.favorites.modules.map(module => (
                    <li 
                      key={module.code} 
                      className="py-3 px-4 hover:bg-ios-gray6 dark:hover:bg-ios-dark-secondary transition-colors cursor-pointer"
                      onClick={() => handleSelectModule(module)}
                    >
                      <div className="flex justify-between">
                        <div>
                          <p className="font-medium dark:text-ios-dark-text">{module.name}</p>
            ...
            {Object.keys(categorizedModules.byDepartment || {}).length > 0 && (
              <div className="bg-white dark:bg-ios-dark-elevated ios-card shadow-sm rounded-xl overflow-hidden">
                <h2 className="px-4 py-3 font-medium border-b border-ios-gray6 dark:border-ios-dark-border dark:text-ios-dark-text">
                  Modules by Department
                </h2>
                
                <div className="divide-y divide-ios-gray6 dark:divide-ios-dark-border">
                  {Object.keys(categorizedModules.byDepartment).map(deptCode => {
                    const dept = categorizedModules.byDepartment[deptCode];
                    const isUserDepartment = userProfile?.department === deptCode;
                    
                    // Check after filters are applied if we have modules to show
                    const modulesToShow = dept.modules.filter(module => {
                      // Apply year filter
                      if (filterOptions.year !== 'all') {
                        const yearMatch = module.code.match(/[A-Z]+(\d)/);
                        if (!yearMatch || yearMatch[1] !== filterOptions.year) return false;
                      }
                      
                      // Apply level filter
                      if (filterOptions.level !== 'all') {
                        const level = parseInt(module.code.match(/[A-Z]+(\d)/)?.[1] || '0');
                        if (filterOptions.level === 'undergraduate' && (level < 1 || level > 3)) return false;
                        if (filterOptions.level === 'postgraduate' && level < 5) return false;
                      }
                      
                      // Apply semester filter
                      if (filterOptions.semester !== 'all' && !module.code.includes(`S${filterOptions.semester}`)) {
                        return false;
                      }
                      ...
                      <div key={deptCode} className={`group ${isUserDepartment ? 'bg-ios-blue-50 dark:bg-ios-teal dark:bg-opacity-5' : ''}`}>
                        <div className="px-4 py-3 font-medium flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <h3 className="dark:text-ios-dark-text">{dept.name}</h3>
                          </div>
                          <span className="text-sm text-ios-gray dark:text-ios-dark-text-tertiary">{modulesToShow.length} modules</span>
                        </div>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-3 bg-ios-gray6 dark:bg-ios-dark-secondary">
                          {modulesToShow.slice(0, 6).map(module => (
                            <div 
                              key={module.code} 
                              className="p-3 bg-white dark:bg-ios-dark-elevated rounded-lg hover:shadow transition-shadow cursor-pointer"
                              onClick={() => handleSelectModule(module)}
                            >
                              <div className="flex justify-between">
                                <div>
                        ...
                        {modulesToShow.length > 6 && (
                          <div className="px-4 py-2 text-center border-t border-ios-gray6 dark:border-ios-dark-border">
                            <button 
                              className="text-sm text-ios-blue dark:text-ios-teal hover:underline"
                              onClick={() => {
                                // Expand view logic here
                                console.log(`Show all ${modulesToShow.length} modules for ${dept.name}`);
                              }}
                            >
                              View all {modulesToShow.length} modules →
                            </button>
                          </div>
                        )}
                      </div>
            ...
            {filteredModules.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-4">
                {filteredModules.map(module => (
                  <div 
                    key={module.code} 
                    className="p-3 bg-ios-gray6 dark:bg-ios-dark-secondary rounded-lg hover:shadow transition-shadow cursor-pointer"
                    onClick={() => handleSelectModule(module)}
                  >
                    <div className="flex justify-between">
                      <div>
                        <p className="font-medium dark:text-ios-dark-text">{module.name}</p>
                        <p className="text-sm text-ios-gray dark:text-ios-dark-text-secondary">{module.code}</p>
                        <p className="text-xs text-ios-gray dark:text-ios-dark-text-tertiary truncate max-w-xs mt-1">
              ...
              <div className="p-8 text-center">
                <p className="text-ios-gray dark:text-ios-dark-text-secondary mb-2">No modules found</p>
                {searchTerm && <p className="text-sm text-ios-gray dark:text-ios-dark-text-tertiary">Try a different search term</p>}
              </div>
            )}
          </div>
        )}
